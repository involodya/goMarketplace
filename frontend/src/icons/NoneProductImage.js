export const NoneProductImage = ({className}) => {
    return (
        <svg
            className={`vuesax-bold-gallery-1 ${className}`}
            fill="none"
            height="60"
            viewBox="0 0 60 60"
            width="60"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path
                className="path"
                d="M6.45 47.5252L6.4 47.5752C5.725 46.1002 5.3 44.4252 5.125 42.5752C5.3 44.4002 5.775 46.0502 6.45 47.5252Z"
                fill="#85A8F8"
            />
            <path
                className="path"
                d="M22.5 25.9498C25.7861 25.9498 28.45 23.2859 28.45 19.9998C28.45 16.7137 25.7861 14.0498 22.5 14.0498C19.214 14.0498 16.55 16.7137 16.55 19.9998C16.55 23.2859 19.214 25.9498 22.5 25.9498Z"
                fill="white"
            />
            <path
                className="path"
                d="M40.475 5H19.525C10.425 5 5 10.425 5 19.525V40.475C5 43.2 5.475 45.575 6.4 47.575C8.55 52.325 13.15 55 19.525 55H40.475C49.575 55 55 49.575 55 40.475V34.75V19.525C55 10.425 49.575 5 40.475 5ZM50.925 31.25C48.975 29.575 45.825 29.575 43.875 31.25L33.475 40.175C31.525 41.85 28.375 41.85 26.425 40.175L25.575 39.475C23.8 37.925 20.975 37.775 18.975 39.125L9.625 45.4C9.075 44 8.75 42.375 8.75 40.475V19.525C8.75 12.475 12.475 8.75 19.525 8.75H40.475C47.525 8.75 51.25 12.475 51.25 19.525V31.525L50.925 31.25Z"
                fill="white"
            />
        </svg>
    );
};
